...
function AutomatorPaginationCreateModalForm(size, titulo, formulario, acao = '', id = null, callback = null) {
    ...
    AutomatorCreateSecurityConfirmationModal({
      ...
      successCallback: function(context) {
        ...
        if(context.parentFormEl) {
          ...
          context.parentFormEl.setAttribute('data-submit', 'true');
          context.parentFormEl.setAttribute('data-automator-form-changed', 'false');

          $(window).off('beforeunload.AutomatorModalFormChanged');

          context.parentFormEl.submit();
  ...
  function AutomatorPaginationModalCreateForm(response, recordData = {}) {
    ...
    if(size === null || size === undefined || size === '') {
      ...
      if(normalizedSize == 'default' || normalizedSize == 'normal' || normalizedSize == 'md') {
        modalSizeClass = '';
      } else if(normalizedSize == 'sm' || normalizedSize == 'small') {
        modalSizeClass = 'modal-sm';
      } else if(normalizedSize == 'lg' || normalizedSize == 'large') {
        modalSizeClass = 'modal-lg';
      } else if(normalizedSize == 'xl' || normalizedSize == 'extra-large') {
        modalSizeClass = 'modal-xl';
      } else if(normalizedSize == 'fullscreen') {
        modalSizeClass = 'modal-fullscreen';
      } else if(normalizedSize.indexOf('modal-') === 0) {
        modalSizeClass = normalizedSize;
      } else {
        modalSizeClass = 'modal-' + normalizedSize;
      }
    ...
    modalHTML += '<div class="modal fade automator-pagination-form-modal" id="' + modalID + '" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">';
      modalHTML += '<div class="modal-dialog' + modalSizeClass + ' modal-dialog-centered modal-dialog-scrollable">';
...
function AutomatorPageFormCreateSecurityConfirmation(formEl = null, submitterEl = null) {
  ...
  if(submitterEl) {
    ...
    submitterName  = submitterEl.getAttribute('name') || '';
    submitterValue = submitterEl.getAttribute('value') || '';
  ...
  AutomatorCreateSecurityConfirmationModal({
    ...
    successCallback: function(context) {
      ...
      if(context.parentFormEl) {
        ...
        if(context.submitterName != '') {

          var submitterInput = context.parentFormEl.querySelector('input[type="hidden"][data-automator-submitter-input="true"][name="' + context.submitterName + '"]');
          ...
          if(!submitterInput) {
            ...
            submitterInput.name = context.submitterName;
            submitterInput.setAttribute('data-automator-submitter-input', 'true');

            context.parentFormEl.appendChild(submitterInput);

          }

          submitterInput.value = context.submitterValue;
...
function AutomatorInitPageFormSecurityValidation() {

  var selector = 'form[data-form-validade="true"], form[data-form-validate="true"]';

  $(document).off('submit.AutomatorPageFormSecurityValidation', selector).on('submit.AutomatorPageFormSecurityValidation', selector, function(e) {
    ...
    if(formEl.hasAttribute('data-form-validade')) {
      formValidate = AutomatorNormalizeBoolean(formEl.getAttribute('data-form-validade'));
    } else if(formEl.hasAttribute('data-form-validate')) {
      formValidate = AutomatorNormalizeBoolean(formEl.getAttribute('data-form-validate'));
    }
    ...
    var submitterEl = null;

    if(e.originalEvent && e.originalEvent.submitter) {
      submitterEl = e.originalEvent.submitter;
    } else if(document.activeElement && document.activeElement.form == formEl) {
      submitterEl = document.activeElement;
    }
    ...
    AutomatorGetActionStatus(function() {
      ...
      AutomatorSetActionStatus(true, function() {
        ...
        AutomatorPageLoader('show', function() {
          ...
          AutomatorPageFormCreateSecurityConfirmation(formEl, submitterEl);
...
$(function() {
  ...
  AutomatorInitPageFormSecurityValidation();
